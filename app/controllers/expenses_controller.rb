class ExpensesController < ApplicationController
        ...
        def index
                if params[:months]
                        @expenses=Expense.all.where('extract(month from created_at)=?',Date: :MONTHNAMES.index(params[:month]), author_id: current_user)
                else
                ...
                @expenses_by_day=@expenses.order(created_at: :asc).group_by {|expence|  expence.created_at.strftime("%A-%m-%B") }
        end
